...
class UserFollowingLayout extends StatelessWidget {
  const UserFollowingLayout({super.key, required this.username});
...
class UserListFollowing extends StatefulWidget {
  ...
  @override
  State<UserListFollowing> createState() => _UserListFollowingState();
}

class _UserListFollowingState extends State<UserListFollowing> {
  ...
  void _fetchMoreUsers({bool initialLoad = false}) {
    if (!_isLoadingMore) {
      ...
      BlocProvider.of<GalleryBloc>(context)
          .add(UserFetchFollowing(widget.username, lastId ?? 0));
    }
  }

  @override
  void didUpdateWidget(UserListFollowing oldWidget) {
