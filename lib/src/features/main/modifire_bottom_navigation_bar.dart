// Код был скопирован c "flutter/lib/src/material/bottom_navigation_bar.dart"
// Был добавлен новый параметр selectedItemBackgroundColor, который принимает цвет активной ячейки
// (строчки 241, 258, 554). Используется этот параметр на строчке 639.
//
// Изменен widget на 639 строчке.
// Изменены Padding параметры на 647.
...
class ModifiedBottomNavigationBar extends StatefulWidget {
  ...
  @override
  State<ModifiedBottomNavigationBar> createState() =>
...
class _BottomNavigationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget result = Material(
      ...
      color: selected ? selectedItemBackgroundColor : Colors.transparent,
      child: InkWell(
        onTap: onTap,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<ColorTween?>('labelColorTween', labelColorTween),
    );
    properties.add(
      DiagnosticsProperty<ColorTween?>('iconColorTween', iconColorTween),
    );
    ...
    properties.add(
      EnumProperty<BottomNavigationBarLandscapeLayout>('layout', layout),
    );
    properties.add(ColorProperty(
        'selectedItemBackgroundColor', selectedItemBackgroundColor));
  }
    ...
    if (item.label != null) {
      ...
      text = MediaQuery(
        data: mediaQueryData.copyWith(
          textScaleFactor: math.min(1, mediaQueryData.textScaleFactor),
        ),
        child: text,
      );
    }
...
class _BottomNavigationBarState extends State<ModifiedBottomNavigationBar>
  ...
  BottomNavigationBarType get _effectiveType => BottomNavigationBarType.fixed;
  ...
  @override
  void didUpdateWidget(ModifiedBottomNavigationBar oldWidget) {
    ...
    for (var i = 0; i < widget.items.length; i++) {
      ...
      tiles.add(
        _BottomNavigationTile(
          ...
          enableFeedback:
              widget.enableFeedback ?? bottomTheme.enableFeedback ?? false,
          onTap: () {
            widget.onTap?.call(i);
          },
          labelColorTween:
